...
KEYWORD_MAPPING = {
    ...
    # --- Kategori Kemanusiaan/Pariwisata/Kuliner (Fokus Bahasa & Komunikasi diperluas) ---
    "komunikasi": "komunikasi media jurnalistik broadcast public relations PR",
    "emosi": "psikologi perilaku mental manusia konseling organisasi", # Tambahan untuk psikologi
    "orang": "psikologi perilaku mental manusia konseling organisasi", # Tambahan untuk psikologi
    ...
    "bahasa": "bahasa inggris mandarin sastra translation interpreter", # Tambahan untuk bahasa
    
    # --- Kategori Lain ---
    "olahraga": "sport fitness kesehatan health wellness management",
    
    # Menghapus duplikasi yang tidak perlu (gambar, dagang, coding, angka, traveling, memasak)
}

# --- DATA DESKRIPSI JURUSAN (LAMA) ---
...
# --- FUNGSI BARU UNTUK MEMASTIKAN REKOMENDASI JIKA KEYWORD DIPILIH ---
# Ini penting untuk mengatasi kasus "saya suka menangis" + "film"
def forced_recommendation_based_on_keyword(selected_keywords, df_filtered):
    ...
    yang dipilih, jika pencarian utama tidak menghasilkan apa-apa.
    """
    program_match = []
    
    # Map keyword ke Program Studi yang Relevan
    keyword_to_program = {
        'film': ['Ilmu Komunikasi', 'Desain Komunikasi Visual'],
        'komunikasi': ['Ilmu Komunikasi'],
        'emosi': ['Psikologi'],
        'orang': ['Psikologi', 'Ilmu Komunikasi'],
        'seni': ['Desain Komunikasi Visual', 'Desain Interaktif'],
        'ngoding': ['Informatika', 'Data Science', 'Sistem Informasi'],
        'bisnis': ['Manajemen', 'Bisnis Digital'],
        'hitung': ['Akuntansi', 'Manajemen'],
        'kuliner': ['Hospitality dan Pariwisata'],
        'data': ['Data Science', 'Informatika'],
        # ... dan seterusnya untuk semua keyword utama
    }
    ...
    if unique_programs:
        # Filter DataFrame untuk Program Studi yang relevan
        df_forced = df_filtered[df_filtered['Program'].isin(unique_programs)].copy()
        
        # Ambil 5 mata kuliah acak dari program-program ini
        if not df_forced.empty:
            if len(df_forced) > 5:
                return df_forced.sample(5)[['Program', 'Semester', 'Course']]
            else:
                return df_forced[['Program', 'Semester', 'Course']]
                
    return pd.DataFrame() # Kembalikan DataFrame kosong jika tidak ada program yang cocok

# --- FUNGSI LAIN (TIDAK BERUBAH SECARA ESENSIAL) ---
@st.cache_data
def load_data():
    # ... (logika load_data)
    try:
...
def get_program_description(program_name):
    # ... (logika get_program_description)
...
def get_recommendations(user_query, df_filtered, words_to_remove=None, selected_keywords=None, top_n=10):
    ...
    # Tambahan: Jika skor kemiripan tertinggi sangat rendah (misalnya < 5%) DAN ada keyword yang dipilih,
    # kita anggap pencarian gagal dan menggunakan forced_recommendation
...
# --- BAGIAN UI/UX (main_app) ---
# Tidak ada perubahan besar selain pemanggilan fungsi,
# tetapi ditampilkan kembali untuk kelengkapan.
